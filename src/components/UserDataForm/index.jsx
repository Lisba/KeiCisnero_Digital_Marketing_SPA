...
const UserDataForm = ({ closeModal, title = '¡Cuéntame de ti!', message = true, name = true }) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (nameResult && emailResult) {
      emailjs
        .sendForm('contact_service', 'contact_form', event.target, 'user_iHcAhH2HFpHfkspecFfaT')
        .then(
          () => {
            closeModalHandler();
          },
    ...
    <div id='modal' className='modal-window' id='modal-window'>
      <div className='modal_content' id='modal_content'>
        <div className='modal-header' id='modal-header'>
          ...
          <span className='header-close' id='close' onClick={closeModalHandler}>
            &times;
          </span>
        </div>
        <form id='contact_form' className='contact-form' onSubmit={handleSubmit}>
          ...
          {message && (
            <div className='contactFormField'>
              <label>
                Mensaje<span className='textarea-label-span'> - opcional</span>
              </label>
